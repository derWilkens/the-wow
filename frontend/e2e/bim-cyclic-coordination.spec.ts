...
test.describe('bim cyclic coordination', () => {
  ...
  test('models a BIM cyclic coordination workflow with tools, data objects, gateways, and detailed subflows', async ({
    ...
    try {
      ...
      await expect(page.getByTestId(`subprocess-trigger-${step3Id}`)).toHaveAttribute('data-subprocess-state', 'linked')
      await expect(page.getByTestId(`subprocess-trigger-${step4Id}`)).toHaveAttribute('data-subprocess-state', 'linked')
      await expect(page.getByTestId(`subprocess-trigger-${step6Id}`)).toHaveAttribute('data-subprocess-state', 'linked')
      await expect(page.getByTestId(`subprocess-trigger-${step7Id}`)).toHaveAttribute('data-subprocess-state', 'linked')
      ...
      await page.getByTestId(`subprocess-trigger-${step6Id}`).click()
